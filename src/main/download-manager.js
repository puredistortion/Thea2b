...
class DownloadManager {
    constructor() {
        this.downloads = new Map(); // Track ongoing downloads by ID
    }
    ...
    async startDownload(url, cookies = [], outputDir, progressCallback) {
        ...
        try {
            ...
            if (!cookies.length) {
                electronLog.info('No cookies provided. Attempting to fetch automatically...');
                cookies = await this.fetchCookies(url);
                electronLog.info(`Automatically fetched ${cookies.length} cookies for URL: ${url}`);
            }
            ...
            if (cookies.length > 0) {
                ...
                electronLog.warn('No cookies provided or fetched for this download.');
            }
